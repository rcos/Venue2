  ...
  methods: {
    async getEvent() {
      ...
      if (
        ...
        this.submission_window_ongoing
      ) {
    ...
    setEventStatus() {
      ...
      if (current_time <= event_start_time) this.event_is_pending = true;
      else if (
    ...
    setSubmissionWindowStatus() {
      ...
      else this.submission_window_closed = true;
    },
    showQR(qr_data) {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          function(position) {
            ...
            //check if submission.location is inside event.geofence
            if (
              google.maps.geometry.poly.containsLocation(
                new google.maps.LatLng(pos.lat, pos.lng),
                new google.maps.Polygon({ paths: self.event.geofence })
              )
            ) {
              ...
              QRScanner.initiate({
                onResult: result => {
                  ...
                  let submission = {
                    event: self.event,
                    submitter: self.$store.state.user.current_user,
                    time: Date(),
                    code: result,
                    location: pos
                  };
