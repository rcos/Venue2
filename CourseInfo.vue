  ...
  <div>
    <show-at breakpoint="large">
      <div class="venue-body-container">
        ...
        <div class="title">
          <router-link
            v-if="is_instructor || is_ta"
            :to="{
              name: 'new_lecture',
              params: {
                course_id: is_instructor ? course._id : $route.params.id
              }
            }"
            tabindex="-1"
          >
            <div
              class="inline-block big-button"
              :style="{ float: 'right' }"
              tabindex="0"
            >
              Create New Lecture for {{ course.prefix }} {{ course.suffix }}
            </div>
          </router-link>
        </div>
        <CourseInfoTitle
          :course="typeof course == typeof {} ? course : {}"
          class="inline-block"
        ...
        <div>
          <div
            v-if="is_instructor"
            class="section-select-container float-right"
          >
            <label id="section_select_label">Section(s):</label>
            <select
              v-model="selected_section"
              class="form-control"
              aria-labelledby="section_select_label"
              @change="onSectionChange"
            >
              <option :value="'all'" selected>All</option>
              <option
                v-for="(section, i) in sorted_sections"
                :key="i"
                :value="section._id"
                >{{ section.name }}</option
              >
        ...
        <InstructorAttendanceHistory
          v-if="
            (is_instructor || is_ta) &&
              lectures_loaded &&
              sorted_lectures[selected_section]
          "
          :lectures="sorted_lectures[selected_section].lectures"
          :timeline="sorted_lectures[selected_section].timeline"
          :students="
            selected_section == 'all'
              ? course_students
              : sections[selected_section].students
          "
          :scores_loaded="scores_loaded"
        />
        <StudentAttendanceHistory
          v-else-if="
            is_student && lectures_loaded && sorted_lectures[section_id]
          "
          :lectures="sorted_lectures[section_id].lectures"
          :timeline="sorted_lectures[section_id].timeline"
          :scores_loaded="scores_loaded"
        />
    ...
    <hide-at breakpoint="large">
      <div>
        ...
        <router-link
          ...
          :to="{
            name: 'new_lecture',
            params: { course_id: is_instructor ? course._id : $route.params.id }
          }"
          tabindex="-1"
        >
          <div class="inline-block big-button mobile" tabindex="0">
            Create New Lecture for {{ course.prefix }} {{ course.suffix }}
          </div>
        </router-link>
        <div class="courseinfo-attendance-listing">
          <div
            v-if="is_instructor"
            class="section-select-container mobile float-right"
          >
            <label id="section_select_label">Section(s):</label>
            <select
              v-model="selected_section"
...
import {
  ...
  getRecentLectures,
  getUpcomingLectures,
  getActivePlaybackLectures,
  getPastLectures
} from "@/services/GlobalFunctions.js";
  ...
  methods: {
    async getAllSections() {
      SectionAPI.getSectionsForCourse(this.course_id)
        .catch(err => {
          console.log(`Problem getting sections for course ${this.course_id}`);
          console.log(err);
        })
        .then(response => {
    ...
    getCurrentUser() {
      ...
      this.is_instructor = this.current_user.instructor_courses.includes(
        this.$route.params.id
      );
      this.is_ta = this.current_user.ta_sections.includes(
        this.$route.params.id
      );
      this.is_student = this.current_user.student_sections.includes(
        this.$route.params.id
      );
    },
    ...
    async getAllLecturesForCourse() {
      LectureAPI.getLecturesForCourse(this.course_id).then(response => {
    ...
    async getAllLecturesForSection() {
      LectureAPI.getLecturesForSection(this.section_id).then(response => {
        this.all_lectures = response.data;
        this.all_lectures.sort((a, b) =>
          a.start_time > b.start_time ||
          a.playback_submission_start_time > b.playback_submission_start_time ||
    ...
    sortLecturesBySectionAndBuildTimeLine(options) {
          ...
          if (undefined != this.sections[sectID]) {
            if (undefined == lect.students) {
              ...
              for (let i = 0; i < this.sections[sectID].students.length; i++) {
                if (
                  !lect.students.includes(this.sections[sectID].students[i])
                ) {
      ...
      Object.keys(sorted).forEach(sectID => {
        ...
        sorted[sectID].lectures.sort((a, b) =>
          a.start_time > b.start_time ? 1 : -1
        );
        for (let i = 0; i < sorted[sectID].lectures.length; i++) {
          let lect = sorted[sectID].lectures[i];
          if (
            undefined == lect.start_time ||
            "Invalid Date" == lect.start_time ||
            null == lect.start_time
          ) {
    ...
    async calculateInstructorAttendances() {
      let promise_tracker = [];
      this.all_lectures.forEach(lecture_ => {
        promise_tracker.push(
          LectureSubmissionAPI.getLectureSubmissionsForLecture(lecture_._id)
            .catch(err => {
              console.log(
                "error retrieving lecture submissions for lecture " +
                  lecture_._id
              );
              console.log(err);
            })
      ...
      Promise.all(promise_tracker).then(res => {
    ...
    calculateStudentAttendances() {
      if (this.sorted_lectures[this.section_id]) {
        ...
        this.sorted_lectures[this.section_id].lectures.forEach(lecture_data => {
          if (this.is_student) {
            promise_tracker.push(
              LectureSubmissionAPI.getOrMake(
                lecture_data._id,
                this.current_user._id
              )
                .catch(err => {
                  console.log(
                    `error retrieving lecture submission for student ${this.current_user._id}`
                  );
                  console.log(err);
                })
                .then(response => {
                  if (response.data == null || response.data == []) {
                    ...
                    lecture_data.color = this.perc2color(
...
/*:root {
  ...
  --course-attendance-percent-medium: #b95726;
  --course-attendance-percent-good: #04852f;
