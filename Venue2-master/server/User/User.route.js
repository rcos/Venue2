...
userRoutes.route('/submit_settings').post( function(req, res) {

  let form = req.body.user;
  console.log(`${form.dark_mode}`)
  ...
  User.findOne({ _id: req.body.user }, (err, user) => {
    if (err || user == null) {
      ...
      user.dark_mode = form.dark_mode;
      user.save();
      console.log(`set ${ user.first_name }'s dark mode preference to ${ user.dark_mode }`);
      console.log(`User leaves the POST method with ${user}`);
      res.json({ user });
    }
  });
});
