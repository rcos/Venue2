function isLive(lecture) {
  ...
  return (
    current_time >= new Date(lecture.start_time) &&
...
function isUpcoming(lecture) {
  ...
  if (lecture.start_time) {
    return (
      new Date(lecture.start_time) > current_time &&
      new Date(lecture.start_time) <= one_week_from_now
    );
  } else {
    return (
      new Date(lecture.playback_submission_start_time) > current_time &&
      new Date(lecture.playback_submission_start_time) <= one_week_from_now
    );
  }
}
...
function isActivePlayback(lecture) {
  let current_time = new Date();
  return (
    current_time >= new Date(lecture.playback_submission_start_time) &&
    current_time <= new Date(lecture.playback_submission_end_time)
  );
}
...
export function getLiveLectures(lectures) {
  ...
  lectures.forEach(lecture => {
    if (isLive(lecture)) live_lectures.push(lecture);
  });
...
export function getUpcomingLectures(lectures) {
  ...
  lectures.forEach(lecture => {
    if (isUpcoming(lecture)) upcoming_lectures.push(lecture);
  });
...
export function getPastLectures(lectures) {
  let past_lectures = [];
  lectures.forEach(lecture => {
    if (isPast(lecture)) past_lectures.push(lecture);
  });
...
export function getActivePlaybackLectures(lectures) {
  ...
  lectures.forEach(lecture => {
    if (isActivePlayback(lecture)) active_playback_lectures.push(lecture);
  });
...
export function getRecentLectures(lectures) {
  ...
  past_lectures.forEach(lecture => {
    if (isRecent(lecture)) recent_lectures.push(lecture);
  });
