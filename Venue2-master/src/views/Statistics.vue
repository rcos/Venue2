  ...
  <div id="stats-container">
    <div id="stats-left">
      <div id="side-panel" class="stats-panel">
        <div
          class="row side-panel-section justify-content-end"
          v-if="courses.all.length > 0"
        >
        ...
        <div
          class="row side-panel-section justify-content-end"
          v-if="sections.filtered.length > 0"
        >
          <div class="col selector-label">
            Sections:
          </div>
          <MultiSelectDropdown
            ...
            :options="sections.filtered"
            sortBy="name"
            @update="handleSectionsChange"
            :n="1"
            ref="sectionsSelector"
          />
        </div>
        <div
          class="row side-panel-section justify-content-end"
          v-if="lectures.filtered.length > 0"
        >
          <div class="selector-label">
            Lectures:
          </div>
          <MultiSelectDropdown
            id="lectures-selector"
            :options="lectures.filtered"
            sortBy="title"
            @update="handleLecturesChange"
            :n="2"
            ref="lecturesSelector"
          />
        </div>
        <div
          class="row side-panel-section justify-content-end"
          v-if="students.filtered.length > 0"
        >
          <div class="col selector-label">
            Students:
          </div>
          <MultiSelectDropdown
            ...
            :options="students.filtered"
            sortBy="last_name"
            :displayAs="['last_name', 'first_name']"
            @update="handleStudentsChange"
            :n="3"
            ref="studentsSelector"
          />
        </div>
      </div>
    </div>
    <div id="stats-right">
      <div id="stats-right-top">
        <div id="top-panel" class="stats-panel">
          <ToggleSwitch
            :class="
              lectures.active.length != 1 && courses.active ? '' : 'hidden'
            "
            label="Cumulative"
            :start="stacked"
            @toggle="handleToggleStacked"
          />
          <ToggleSwitch
            :class="
              lectures.active.length == 1 && courses.active ? '' : 'hidden'
            ...
            :start="half"
            :disabled="polls"
            @toggle="handleToggleHalf"
          />
          <ToggleSwitch
            :class="
              lectures.active.length > 0 && courses.active ? '' : 'hidden'
            "
            label="Polls"
            :start="polls"
            @toggle="handleTogglePolls"
          />
        </div>
      </div>
      <div id="stats-right-bottom">
        <div id="stats-render">
          <div v-if="lectures.active.length > 0 && polls">
            <canvas
              v-for="(poll, i) in playbackPolls.displayed"
              :id="'pollsChart_' + i"
              :key="i"
            ></canvas>
          </div>
          ...
          <div v-else-if="sections.active.length > 0">
            <canvas
              v-for="section in sections.active"
              :id="'sectionChart_' + section.name"
              :key="section.name"
            ></canvas>
          </div>
...
export default {
  ...
  methods: {
    fetchData() {
      CourseAPI.getInstructorCourses(this.current_user._id).then(res => {
        this.courses.all = res.data;
        let allStudents = [];
        this.courses.all.forEach(course => {
          SectionAPI.getSectionsForCourse(course._id).then(res2 => {
            this.sections.all = this.sections.all.concat(res2.data);
          });
          allStudents = allStudents.concat(
            new Promise((resolve, reject) => {
              UserAPI.getStudentsForCourse(course._id).then(res2 => {
                let students = [];
      ...
      LectureAPI.getLecturesForUser(this.current_user._id, "none").then(res => {
        this.lectures.all = res.data;
        let all_submissions = [];
        let all_playbackpolls = [];
        this.lectures.all.forEach(lecture => {
          all_submissions = all_submissions.concat(
            new Promise((resolve, reject) => {
              LectureSubmissionAPI.getLectureSubmissionsForLecture(
                lecture._id
              ).then(res => {
                let submissions = [];
          ...
          all_playbackpolls = all_playbackpolls.concat(
            new Promise((resolve, reject) => {
              PlaybackPollAPI.getByLecture(lecture._id).then(res => {
                let polls = [];
      ...
      if (this.lectures.active.length > 0) {
        ...
        this.lectures.active.forEach(lecture => {
          ...
          lecture.sections.forEach(sectID => {
            let section = this.sections.all.find(
              section => section._id == sectID
            );
          ...
          let subsForLecture = this.lectureSubmissions.filtered.filter(
            a => a.lecture == lecture._id
          );
          ...
          if (Object.keys(lecture.students).length > 0) {
            ...
            if (lecture.start_time) {
              ...
              lecturesAttendance[lecture._id].date =
                lecture.playback_submission_start_time;
            }
          }
        });
        ...
        if (this.polls) {
          ...
          lectIDs.sort((a, b) => {
            lecturesAttendance[a].obj.start_time >
              lecturesAttendance[b].obj.start_time ||
            lecturesAttendance[a].obj.playback_submission_start_time >
              lecturesAttendance[b].obj.playback_submission_start_time ||
            lecturesAttendance[a].obj.start_time >
              lecturesAttendance[b].obj.playback_submission_start_time ||
            lecturesAttendance[a].obj.playback_submission_start_time >
              lecturesAttendance[b].obj.start_time
        ...
        this.sections.active.forEach(section => {
          ...
          this.lectures.filtered.sort((a, b) =>
            ...
            a.playback_submission_start_time >
              b.playback_submission_start_time ||
            a.start_time > b.playback_submission_start_time ||
            a.playback_submission_start_time > b.start_time
              ? 1
              : -1
          );
            ...
            if (lecture.sections.includes(section._id)) {
              ...
              if (Object.keys(lecture.students).length > 0) {
                ...
                if (lecture.start_time) {
                  ...
                  sections[section.name].dates.push(
                    lecture.playback_submission_start_time
                  );
                } else {
        ...
        this.lectures.filtered.sort((a, b) =>
          a.start_time > b.start_time ||
          a.playback_submission_start_time > b.playback_submission_start_time ||
          a.start_time > b.playback_submission_start_time ||
          a.playback_submission_start_time > b.start_time
            ? 1
            : -1
      ...
      lectureStudents.forEach(studID => {
        ...
        if (studSubmission.live_percent && studSubmission.video_percent) {
          let percentage = Math.max(
            studSubmission.live_percent,
            studSubmission.video_percent
          );
    ...
    processPollsData(submissions) {
      let data = {};
      let n = 0;
      Object.keys(submissions).forEach(lectID => {
        //for each lecture to be displayed
        if (undefined == data[lectID]) {
          data[lectID] = {};
        }
        this.playbackPolls[lectID].forEach((playbackPoll, i) => {
          //for each question per lecture
          ...
          submissions[lectID].forEach(submission => {
            //for each student
            Object.keys(submission.student_poll_answers).forEach(
              poll_retrieval => {
                //for each question answered
                if (
                  (playbackPoll.code && playbackPoll.code == poll_retrieval) ||
                  (playbackPoll.timestamp &&
                    playbackPoll.timestamp == poll_retrieval)
                ) {
                  let student_answers = [];
                  submission.student_poll_answers[poll_retrieval].forEach(
                    (is_marked, k) => {
                      //for each marked option
                      if (is_marked) {
                  ...
                  let idx = data[lectID][playbackPoll._id].labels.findIndex(
                    a => a == stringified
                  );
                  if (idx >= 0) {
    ...
    handleCourseChange(data) {
      ...
      this.getSectionsForCourse(data[0]).then(sections => {
        if (
          undefined != this.sections.filtered &&
          this.sections.filtered.length > 0
        ) {
    ...
    execLecturesForSections(sections) {
      this.getLecturesForSections(sections).then(lectures => {
        let clean = lectures.filter(a => undefined != a);
        if (
          undefined != this.lectures.filtered &&
          this.lectures.filtered.length > 0
        ) {
    ...
    execStudentsForLectures(lectures) {
      this.getStudentsForLectures(lectures).then(students => {
        let noDups = students.uniqueByProp("_id");
        if (
          undefined != this.students.filtered &&
          this.students.filtered.length > 0
        ) {
    ...
    execSubmissionsForLectures(lectures, students) {
      this.getSubmissionsForLectures(lectures).then(submissions => {
