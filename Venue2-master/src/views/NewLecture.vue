    ...
    <form class="new-lecture-form" @submit.prevent="handleAttemptSubmit">
      ...
      <div class="form-group">
        ...
        <div
          ...
          role="status"
          v-if="!course_sections_have_loaded"
        >
        ...
        <div class="input-wrapper">
          ...
          <MultiSelectDropdown
            v-if="course_sections.length > 0"
            :options="course_sections"
            sortBy="name"
        ...
        <div class="input-wrapper">
          ...
          <input
            @click="setAllowLiveSubmissions"
            type="checkbox"
            name="live_submission"
            v-model="allow_live_submissions"
            aria-labelledby="live_submission_label"
            :tabindex="modal_open ? '-1' : '0'"
          />
          <label id="live_submission_label">Live</label><br />
          <input
            @click="setAllowPlaybackSubmissions"
            type="checkbox"
            name="playback_submission"
            v-model="allow_playback_submissions"
            aria-labelledby="playback_submission_label"
            :tabindex="modal_open ? '-1' : '0'"
          />
        ...
        <div v-if="allow_live_submissions">
          <div class="input-wrapper row">
            <div class="time-container">
              ...
              <div
                id="lecture_start"
                class="js-inline-picker"
                aria-labelledby="start_time_label"
                :tabindex="modal_open ? '-1' : '0'"
              ></div>
            </div>
            <div class="time-container">
          ...
          <div class="input-wrapper">
            ...
            <div>
              <div
                ...
                class="checkin-container row"
                :key="i"
              >
                <div class="checkin-number col col-1 my-auto">#{{ i + 1 }}</div>
                <div class="checkin-type col col-5 my-auto">
                  <MultiSelectDropdown
                    ...
                    @update="handleUpdateCheckin"
                    :max="1"
                    :n="i"
                    :ref="'checkinSelector-' + i"
                  />
                </div>
                <div class="checkin-poll col col-4 my-auto">
                  <div v-if="null != polls[i]">
                    <button
                      ...
                      @click="add_poll_index = i"
                      :title="'Edit ' + polls[i].question"
                    >
                      <img
                ...
                <div id="add-poll-modal" v-if="add_poll_index >= 0">
                  <div class="modal-contents">
                    <CreatePoll
                      :playback_only="false"
                      :checkin="add_poll_index"
                      @addPoll="handleAddPoll"
                      :poll="polls[add_poll_index]"
                      @cancel="add_poll_index = -1"
                    />
                  </div>
                </div>
                <div class="checkin-remove col col-2 my-auto">
                  <button
                    type="button"
                    class="btn btn-danger remove-checkin-btn float-right"
                    @click="handleRemoveCheckin(i)"
                  >
                    X
                  </button>
                </div>
                <div
                  class="checkin-options col my-auto"
                  v-if="checkin.activation == 'Pre-set Time'"
                >
                  <label>Start</label>
                  <div
                    :id="'checkin_start_' + i"
                    class="checkin-time js-inline-picker"
                    :tabindex="modal_open ? '-1' : '0'"
                    type="datetime-local"
                  ></div>
                </div>
                ...
                <div
                  class="checkin-options col my-auto"
                  v-else-if="checkin.activation == 'Random Time'"
                >
        ...
        <LectureUploadModal
          ref="uploadmodal"
          v-if="allow_playback_submissions"
          :lecture="lecture"
          :update_lecture="false"
          :shown="modal_open"
          @openstatus="handleModalChange"
        />
        <p class="error_msg" v-if="input_error_message != ''">
          {{ input_error_message }}
        </p>
        <button
          class="btn btn-primary create-lecture-btn"
          :tabindex="modal_open ? '-1' : '0'"
        >
          Create Lecture
        </button>
...
export default {
  ...
  created() {
    if (
      this.$store.state.user.current_user.ta_sections.includes(
  ...
  methods: {
    ...
    handleAttemptSubmit(evt) {
      ...
      let hasType =
        this.allow_live_submissions || this.allow_playback_submissions;
      if (hasTitle && hasSections && hasType) {
        if (this.allow_live_submissions) {
          ...
          let validRange =
            this.start_time_picker.pick().date <=
            this.end_time_picker.pick().date;
          ...
          if (validRange) {
            ...
            if (this.checkins.map(a => a.activation).includes(null)) {
              this.setErrorMessage(
                "One or more checkins missing an activation type"
              );
              allGood = false;
            } else if (presets.map(a => a.start_time).includes(null)) {
              this.setErrorMessage(
                "One or more pre-set check-ins is missing a start time"
              );
              allGood = false;
            } else if (presets.map(a => a.end_time).includes(null)) {
              this.setErrorMessage(
                "One or more pre-set check-ins is missing an end time"
              );
              allGood = false;
            } else if (randoms.map(a => a.minutes).includes(null)) {
              this.setErrorMessage(
                "One or more random check-ins is missing a duration"
              );
              allGood = false;
            } else if (this.hasOverlaps(presets)) {
              this.setErrorMessage(
                "One or more pre-set check-ins are overlapping"
              );
              allGood = false;
            } else if (
              presets.filter(a => a.start_time < this.lecture.start_time)
                .length > 0
            ) {
              this.setErrorMessage(
                "One or more pre-set check-ins starts before the lecture does"
              );
              allGood = false;
            } else if (
              presets.filter(a => a.end_time > this.lecture.end_time).length > 0
            ) {
              this.setErrorMessage(
                "One or more pre-set check-ins ends after the lecture does"
              );
              allGood = false;
            }
          } else if (!validRange) {
    ...
    async addLecture() {
      ...
      if (this.lecture.allow_live_submissions) {
        ...
        this.lecture.checkins = this.lecture.checkins.filter(
          a => !(a.start_time == null && a.activation == "Random Time")
        );
          ...
          if (poll) {
            ...
            poll_promises.push(
              new Promise((resolve, reject) => {
                PlaybackPollAPI.addPoll(poll).then(res => {
                  resolve(res.data);
                });
              })
            );
        ...
        let response = await LectureAPI.addLecture(this.lecture);
        this.lecture = response.data;
        this.$refs["uploadmodal"].updateLectureFromParent(
          this.lecture,
          this.course_id
        );
      }
    ...
    hasOverlaps(checkins) {
      checkins = checkins.concat([]).sort(function(a, b) {
    ...
    generateRandomCheckinTimes() {
      ...
      randoms.forEach(random => {
        ...
        for (let i = 0; i < max_attempts; i++) {
          let attempt_start = randomNumber(
            this.lecture.start_time,
            this.lecture.end_time
          );
          let attempt_end = attempt_start + random.minutes * 60 * 1000;
          let joined = done.concat([
            { start_time: attempt_start, end_time: attempt_end }
          ]);
    ...
    setAllowLiveSubmissions() {
      ...
      this.$nextTick(() => {
        let self = this;
        this.start_time_picker = new Picker(
          document.querySelector("#lecture_start"),
          {
            inline: true,
            controls: true,
            headers: true
        ...
        this.end_time_picker = new Picker(
          document.querySelector("#lecture_end"),
          {
            inline: true,
            controls: true,
            headers: true
    ...
    updatePickers() {
      ...
      this.checkins.forEach((checkin, i) => {
        if (checkin.activation == "Pre-set Time") {
          ...
          pickers.start = new Picker(
            document.querySelector("#checkin_start_" + i),
            {
              inline: true,
              rows: 1,
              headers: true,
          ...
          pickers.end = new Picker(
            document.querySelector("#checkin_end_" + i),
            {
              inline: true,
              rows: 1,
              headers: true,
