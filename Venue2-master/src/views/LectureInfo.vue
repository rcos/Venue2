      ...
      <div class="lecture-info-container mobile">
        ...
        <div v-else>
          <InstructorLectureInfo
            ...
            :lecture="lecture"
            :is_instructor="is_instructor || is_ta"
          />
          <StudentLectureInfo
            v-else-if="is_student && polls_loaded"
            :lecture="lecture"
            :is_instructor="is_instructor || is_ta"
            :polls="polls"
          />
...
export default {
  ...
  methods: {
    async getLecture() {
      ...
      this.is_instructor = this.current_user.instructor_courses.some(
        a => a == this.lecture.sections[0].course._id
      );
      this.is_ta = this.current_user.ta_sections.some(a =>
        this.lecture.sections.map(b => b._id).includes(a)
      );
      this.is_student = this.current_user.student_sections.some(a =>
        this.lecture.sections.map(b => b._id).includes(a)
      );
      PlaybackPollAPI.getByLecture(this.lecture_id).then(res => {
        this.polls = res.data;
