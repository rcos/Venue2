      ...
      <div id="lecture-info-container">
          ...
          <show-at breakpoint="small">
            ...
            <div id="lecture-data">
              <div class="row main-info">
                ...
                <h5 class="dept-and-number">
                  {{ lecture.sections[0].course.dept }}
                  {{ lecture.sections[0].course.course_number }}
                </h5>
                <h5>
                  Sections:
                  <a
                    class="section-numbers"
                    v-for="(section, i) in lecture.sections"
                    :key="i"
                    >{{ section.name }}</a
                  >
                </h5>
        ...
        <div id="lecture-attendance-section">
          <h1>
            Attendance
            <button
              v-if="show_checkin_qr != -1 && is_instructor"
              class="btn btn-secondary show-qr-btn"
              @click="showQR"
            >
              Show QR
            </button>
            <LectureUploadModal
              v-if="
                Date.now() > new Date(lecture.end_time) &&
            ...
            <button
              v-else-if="
                show_checkin_qr != -1 &&
                  !is_instructor &&
                  self_submission_count <= show_checkin_qr
              "
              class="btn btn-secondary scan-qr-btn"
              @click="showQRPreview"
            >
              Attend this lecture
            </button>
            <router-link
              v-else-if="
                (Date.now() > new Date(lecture.end_time) ||
                  undefined == lecture.end_time) &&
                  lecture.video_ref.includes('.')
              "
              :to="{
                name: 'lecture_playback',
                params: { lecture_id: lecture._id }
              }"
            >
              <button class="btn btn-secondary watch-recording">
                Watch recording
              </button>
            </router-link>
          </h1>
          <div v-if="is_instructor">
            <div class="tabs">
              <button
                id="live_btn"
                class="tab_btn selected_tab"
                @click="selectTab(0)"
              >
                <h5>
                  Live ({{ Object.keys(live_submissions).length }}/{{
                    all_students.length
                  }})
                </h5>
              </button>
              <button id="playback_btn" class="tab_btn" @click="selectTab(1)">
                <h5>
                  Playback ({{ playback_submissions.length }}/{{
                    all_students.length
                  }})
                </h5>
              </button>
              <button id="absent_btn" class="tab_btn" @click="selectTab(2)">
                <h5>Absent ({{ absent.length }}/{{ all_students.length }})</h5>
              </button>
              <button id="stats_btn" class="tab_btn" @click="selectTab(3)">
                <h5>Statistics</h5>
              </button>
            </div>
            <div v-if="selected_tab == 0" id="live_submit" class="tab_section">
              <div v-if="Object.keys(live_submissions).length > 0">
                <div
                  class="namecard-edging live-color"
                  v-for="(submission, i) in Object.keys(live_submissions)"
                  :key="i"
                >
                  <div class="namecard">
                    {{ live_submissions[submission][0].submitter.first_name }}
                    {{ live_submissions[submission][0].submitter.last_name }}
                    {{ live_submissions[submission][0].submitter.email }}
                    {{
                      (live_submissions[submission].length /
                        lecture.checkins.length) *
                        100
                    }}%
                  </div>
                </div>
            ...
            >
              <div v-if="playback_submissions.length > 0">
                <div
                  class="namecard-edging playback-color"
                  v-for="(submission, i) in playback_submissions"
                  :key="i"
                >
                  <div class="namecard">
            ...
            <div v-if="selected_tab == 2" id="no_submit" class="tab_section">
              <div v-if="absent.length > 0">
                <div
                  class="namecard-edging absent-color"
                  v-for="(absentee, i) in absent"
                  :key="i"
                >
                  <div class="namecard">
          ...
          <div v-else class="student_lecture_attendance_info">
            <div v-if="self_submission_count > 0">
              <div
                v-for="student_lecture_submission in student_lecture_submissions"
              >
                <div v-if="student_lecture_submission.is_live_submission">
                  <p>
                    Live submission was made {{ self_submission_count }} time(s)
                  </p>
                  <p>
                    Live submission time:
                    {{
                      new Date(student_lecture_submission.live_submission_time)
                    }}
                  </p>
                </div>
                <div v-else>
...
export default {
  ...
  created() {
    ...
    this.getLecture();
    if (this.is_instructor) this.getStudentsAndCalcAttendance();
    else this.getStudentLectureSubmissions();
  },
  methods: {
    async getLecture() {
      const response = await LectureAPI.getLectureWithSectionsAndCourse(
        this.lecture_id
      );
    ...
    setLectureStatus() {
      ...
      let lecture_playback_submission_start_time = new Date(
        this.lecture.playback_submission_start_time
      );
      let lecture_playback_submission_end_time = new Date(
        this.lecture.playback_submission_end_time
      );
      if (
        undefined == this.lecture.start_time ||
        undefined == this.lecture.end_time
      ) {
        this.lecture_is_over = true;
      }
      if (
        current_time < lecture_start_time ||
        current_time < lecture_playback_submission_start_time
      )
        this.lecture_is_upcoming = true;
      else if (
        current_time >= lecture_start_time &&
        current_time <= lecture_end_time
      ) {
        this.lecture_is_live = true;
        this.setSubmissionWindowStatus();
      } else this.lecture_is_over = true;
    },
    async getStudentLectureSubmissions() {
      const response = await LectureSubmissionAPI.getLectureSubmissionsForStudent(
        this.lecture_id,
        this.user_id
      );
    ...
    showQR() {
      QRCode.toCanvas(
        document.getElementById("qr_render_area"),
        this.lecture.checkins[this.show_checkin_qr].code,
        function(error) {
    ...
    async createLiveSubmission() {
      ...
      const response = await LectureSubmissionAPI.addLectureSubmission(
        lecture_submission
      );
      this.self_submission_count++;
    ...
    handleAttendanceOverride() {
      let names = document
        .getElementById("attendance_override")
        .value.replace(/\s/g, "")
        .split(",");
...
.tab_btn.selected_tab {
  color: #0078c2;
  border-bottom: 0.2rem solid #0078c2;
}
