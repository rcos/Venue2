<template>
  <div id="dashboard-container">
    <SquareLoader
      role="status"
      v-if="
        !live_lectures_loaded &&
          !playback_lectures_loaded &&
          !recent_lectures_loaded &&
          !upcoming_lectures_loaded
      "
    />
    <div v-else>
      <DashboardSection lecture_type="Live" :lecture_list="live_lectures" />
      <DashboardSection
        lecture_type="Playback"
        :lecture_list="playback_lectures"
      />
      <DashboardSection lecture_type="Recent" :lecture_list="recent_lectures" />
      <DashboardSection
        lecture_type="Upcoming"
        :lecture_list="upcoming_lectures"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getColor(course_info) {
      ...
      if (
        this.courses[course_info._id] == null ||
        this.courses[course_info._id] == undefined
      ) {
        // add the course and
        let next_color_index = 0;
        Object.keys(this.courses).forEach(course_ => {
          if (this.courses[course_].hasOwnProperty("color_index"))
            next_color_index = Math.max(
              next_color_index,
              this.courses[course_].color_index
            );
        });
      ...
      if (!this.courses[course_info._id].hasOwnProperty("color_index"))
        return "grey";
      ...
      if (
        ...
        color_index >= 0 &&
        color_index < this.STATIC_COURSE_COLORS.length
      ) {
    ...
    async getAllLecturesForUser() {
      const response = await LectureAPI.getLecturesForUser(
    ...
    setCheckinWindowStatus(lecture) {
      ...
      for (let i = 0; i < lecture.checkins.length; i++) {
        ...
        if (
          current_time >= current_checkin_start_time &&
          current_time <= current_checkin_end_time
        ) {
      ...
      if (!found_open_checkin_window) lecture.checkin_window_status = "closed";
    },
    sortLiveLecturesByCheckinWindowStatus() {
      ...
      this.live_lectures.forEach(lecture => {
        ...
        else temp_list.push(lecture);
      });
      this.live_lectures = temp_list;
    },
    chooseLecturesToDisplay() {
      let lecture_existence_pairs = [
        ["live", this.live_lectures_exist],
        ["playback", this.playback_lectures_exist],
        ["recent", this.recent_lectures_exist],
