        ...
        <div v-else>
          ...
          <div v-else-if="sections.length > 0">
            <CourseCard
              ...
              v-bind:box_color="getColor(section.course)"
            />
          </div>
          <div v-else>
            <p class="no-container" id="no-courses">No courses</p>
...
export default {
  ...
  created() {
    ...
    if (this.is_instructor) this.getInstructorCourses();
    else this.getSectionsWithCourses();
  },
  methods: {
    getColors() {
    ...
    getColor(course) {
      if (course.color_index == null || course.color_index == undefined)
        return Constants.colors[
    ...
    assignBoxColorsToClassObjects(class_objects) {
      ...
      class_objects.forEach(class_object => {
        ...
        if (box_color_index == this.box_colors.length) box_color_index = 0;
        else box_color_index++;
      });
    }
  }
};
