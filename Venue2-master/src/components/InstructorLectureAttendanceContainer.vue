<template>
  <div>
    <div v-if="current_checkin" id="qr_modal">
      <qrcode
        id="qr_canvas"
        v-bind:value="current_checkin.code"
        :options="{ width: 400 }"
      ></qrcode>
    ...
    <div id="container-header">
      ...
      <div
        v-for="(checkin, i) in lecture.checkins"
        :key="i"
        class="inline-block"
      >
        <button
          v-if="
            checkin.activation != 'Manual Activation' && checkinIsOpen(checkin)
          "
          type="button"
        ...
        >
          <img
            src="@/assets/icons8-qr-code-50.png"
            width="60"
            alt="QR Code"
            aria-label="QR Code"
          />
      ...
      <LectureUploadModal
        v-if="
          lectureIsOver() && !lecture.allow_playback_submissions && polls_loaded
        "
        :lecture="lecture"
        :need_timestamp="polls"
        :update_lecture="true"
      />
      <router-link
        class="header-btn btn btn-secondary"
        v-else-if="lecture.allow_playback_submissions"
        title="Watch Recording"
        :to="{ name: 'lecture_playback', params: { lecture_id: lecture._id } }"
        aria-label="Watch Recording"
      >
        <img
          src="@/assets/icons8-video-64.png"
          width="60"
          alt="Video"
          aria-label="Video"
        />
      ...
      <button
        class="header-btn btn btn-primary"
        @click="download_submitty_csv"
        id="submitty_export"
        title="CSV Export"
      >
        <img
          src="@/assets/icons8-database-export-64.png"
          width="60"
          alt="QR Code"
          aria-label="QR Code"
        />
      ...
      >
        <button
          v-if="checkin.activation == 'Manual Activation'"
          class="btn btn-secondary manualbtn"
          @click="showQR(i)"
        >
          Open Check-in {{ i + 1 }}
        </button>
...
export default {
  ...
  created() {
    ...
    PlaybackPollAPI.getByLecture(this.lecture._id).then(res => {
      this.polls = res.data;
  ...
  methods: {
          ...
          this.$nextTick(function() {
            ...
            video.addEventListener("loadedmetadata", function() {
              if (
                ...
                if (
                  self.lecture.checkins[i].end_time &&
                  Date.parse(self.lecture.checkins[i].end_time) - Date.now() > 0
                ) {
                  setTimeout(function() {
                    if (
                      document.pictureInPictureEnabled &&
                    ...
                    location.reload();
                  }, Date.parse(self.lecture.checkins[i].end_time) -
                    Date.now());
                }
              }
            });
    ...
    checkinIsOpen(checkin) {
      let now = new Date();
      return (
        Date.parse(checkin.start_time) <= now &&
        Date.parse(checkin.end_time) >= now
      );
    },
    lectureIsOver() {
      return (
        !this.lecture.end_time || Date.parse(this.lecture.end_time) < Date.now()
      );
    },
    getManualCheckins() {
      return this.lecture.checkins.filter(
        a => a.activation == "Manual Activation"
      );
    },
    ...
    download_submitty_csv() {
      ...
      SectionAPI.getSectionsForCourse(this.lecture.sections[0].course._id).then(
        res => {
          ...
          this.all_students.forEach(function(student) {
            course_sections.forEach(function(section) {
              if (section.students.includes(student._id)) {
                ...
                let submission = self.submissions.find(
                  a => a.submitter._id == student._id
                );

                let percent = 0;
                if (
                  submission &&
                  submission.live_percent &&
                  submission.video_percent
                ) {
                  percent = Math.max(
                    submission.live_percent,
                    submission.video_percent
                ...
                if (submission && submission.student_poll_answers) {
                  let pollsum = 0;
                  stud_data.push(
                    Object.keys(submission.student_poll_answers).length /
                      self.polls.length
                  );
                  self.polls.forEach(poll => {
                    if (
                      poll.code &&
                      submission.student_poll_answers[poll.code]
                    ) {
                      console.log("here");
                      let answers = submission.student_poll_answers[poll.code];
                      if (
                        answers.every(a => poll.correct_answers.includes(a)) &&
                        answers.length == poll.correct_answers.length
                      ) {
                        pollsum += 1;
                      }
                    } else if (
                      poll.timestamp &&
                      submission.student_poll_answers[poll.timestamp]
                    ) {
                      console.log("here");
                      let answers =
                        submission.student_poll_answers[poll.timestamp];
          ...
          let csv =
            "Submission Timestamp,User ID,First Name,Last Name,Registration Section,Submission Type,Grade,Percent Questions Answered,Percent Questions Answered Correctly\n";
          ...
          let downloadname =
            this.lecture.sections[0].course.name +
            "_" +
            this.lecture.title +
            "_attendance.csv";
    ...
    handleEndEarly() {
      LectureAPI.endEarly(this.lecture._id).then(res => {
        location.reload();
      });
    }
  }
