<template>
  <div class="lecture-card">
    <div
      v-if="lecture_type === 'Live'"
      class="lecture-card-background"
      :class="{
        'live-lecture-open': lecture.checkin_window_status === 'open',
        'live-lecture-closed': lecture.checkin_window_status === 'closed'
      }"
    ></div>
    <div
      v-else
      class="lecture-card-background"
      :class="{
        'playback-lecture': lecture_type === 'Playback',
        'recent-lecture': lecture_type === 'Recent',
        'upcoming-lecture': lecture_type === 'Upcoming'
      }"
    ></div>
    <div class="lecture-card-foreground">
      <router-link
        class="lecture_card_link"
        :to="{ name: 'lecture_info', params: { lecture_id: lecture._id } }"
        :aria-label="
          'Lecture Info - ' + lecture_course.name + ' - ' + lecture.title
        "
      >
        <div class="lecture-card-section" id="left-section">
          <p class="course-name">{{ lecture_course.name }}</p>
          <p class="course-title">
            {{ lecture_course.prefix }} {{ lecture_course.suffix }}
          </p>
        </div>
        ...
        <div class="lecture-card-section" id="right-section">
          <div v-if="lecture_type === 'Live'" class="right-container">
            ...
            />
            <p
              v-if="lecture.checkin_window_status === 'open'"
              class="lecture-time-text open-text"
            >
              {{ getTimeToWindowCloseString(false) }}
            </p>
          ...
          <div v-else-if="lecture_type === 'Playback'" class="right-container">
            <img
              ...
              alt="Time Icon"
            />
            <p class="lecture-time-text playback-percentage-text">
              {{ getTimeToWindowCloseString(true) }}
            </p>
          ...
          <div v-else-if="lecture_type === 'Upcoming'" class="right-container">
            ...
            />
            <p
              v-if="lecture.allow_live_submissions"
              class="lecture-time-text upcoming-text"
            >
              {{ getUpcomingTimeString(lecture.start_time) }}
            </p>
            <p v-else class="lecture-time-text upcoming-text">
              {{
                getUpcomingTimeString(lecture.playback_submission_start_time)
              }}
            </p>
          </div>
        </div>
...
export default {
  ...
  methods: {
    getTimeToWindowCloseString(is_playback) {
      ...
      else window_close_time = this.getCurrentCheckinWindowCloseTime();
      ...
      if (diff_days > 0) time_to_close_str += diff_days + "d ";
      if (diff_hours > 0) time_to_close_str += diff_hours + "h ";
      time_to_close_str += diff_minutes + "m ";
      return time_to_close_str;
    },
    getCurrentCheckinWindowCloseTime() {
      return new Date(
        this.lecture.checkins[this.lecture.checkin_index].end_time
      );
    },
