    ...
    <div v-if="todays_events.length > 0">
      ...
      <hide-at breakpoint="small">
        <div class="todays-events-section" id="horizontal-section">
          ...
          <div
            v-for="(event, index) in todays_events"
            class="todays-event-container desktop-event-container"
            v-bind:class="{
              'mleft-one': index === 0,
              'mleft-four': index !== 0
            }"
          >
            <div
              v-if="event.started_today"
              class="todays-event-time desktop-event-time"
            >
              {{ convertToHourMinuteFormat(new Date(event.start_time)) }}
            </div>
            <div v-else class="todays-event-time desktop-event-time">
              {{ convertToMonthDayFormat(new Date(event.start_time)) }}
              {{ convertToHourMinuteFormat(new Date(event.start_time)) }}
            </div>
            <router-link
      ...
      <show-at breakpoint="small">
        <div class="todays-events-section" id="vertical-section">
          ...
          <div
            v-for="(event, index) in todays_events"
            class="todays-event-container mobile-event-container"
          >
            <div
              v-if="event.started_today"
              class="todays-event-time mobile-event-time"
            >
              {{ convertToHourMinuteFormat(new Date(event.start_time)) }}
            </div>
            <div v-else class="todays-event-time mobile-event-time">
              {{ convertToMonthDayFormat(new Date(event.start_time)) }}
              {{ convertToHourMinuteFormat(new Date(event.start_time)) }}
            </div>
...
export default {
  ...
  methods: {
    ...
    isToday(time) {
      ...
      return (
        current_time.getFullYear() === time.getFullYear() &&
