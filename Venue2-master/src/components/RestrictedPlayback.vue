  ...
  <div id="restricted_playback">
    ...
    >
      <source
        ...
        v-bind:src="
          lecture.video_ref +
            '?showinfo=0&enablejsapi=1&origin=https://www.venue-meetings.com'
        "
      ...
      <source
        ...
        v-bind:src="
          lecture.video_ref +
            '?showinfo=0&enablejsapi=1&origin=http://localhost:8080'
        "
        :type="lecture.video_type"
      />
    </video>
      ...
      <div id="poll_contents">
        ...
        >
          ...
          <ol class="optionslist">
            <li
              v-for="(possible_answer, j) in poll.possible_answers"
              :key="j"
              class="row option"
            >
              <p class="optionnumber" aria-labelledby="number_label">
                {{ j + 1 }}:
              </p>
              <p class="optiontext" aria-labelledby="answer_label">
                {{ possible_answer }}
              </p>
              <input
                class="optioncheck"
                type="checkbox"
                :id="'student_answer_' + (i + 1) + '_' + (j + 1)"
                aria-labelledby="correct_label"
              />
            </li>
          </ol>
          <button
            class="answer_btn btn btn-primary"
            :id="'answer_btn_' + (i + 1)"
            @click="answerPoll(i, poll)"
          >
            Submit
          </button>
        </div>
...
export default {
  ...
  mounted() {
    ...
    videojs("video_player", vidOptions, function() {
      ...
      PlaybackPollAPI.getByLecture(self.lecture._id).then(resp => {
        ...
        var bigbrother = () => {
          ...
          if (currTime - self.prevTime < 1.5 && currTime >= self.prevTime) {
            ...
            if (self.submission.video_progress < Math.floor(currTime)) {
              ...
              self.submission.video_percent =
                Math.floor(currTime) / self.lecture.video_length;
            ...
            for (let i = 0; i < self.polls.length; i++) {
              if (currTime > self.polls[i].timestamp) {
                ...
                if (
                  !(
                    (self.polls[i].code &&
                      self.submission.student_poll_answers[
                        self.polls[i].code
                      ]) ||
                    (self.polls[i].timestamp &&
                      self.submission.student_poll_answers[
                        self.polls[i].timestamp
                      ])
                  )
  ...
  methods: {
    ...
    answerPoll(i, poll) {
      ...
      for (let j = 0; j < this.polls[i].possible_answers.length; j++) {
        if (
          document.getElementById("student_answer_" + (i + 1) + "_" + (j + 1))
            .checked
        ) {
