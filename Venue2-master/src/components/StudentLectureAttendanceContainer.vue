<template>
  <div>
    ...
    <div id="qr-scanning-container" v-if="qr_scanning_window_open">
      <button
        @click="qr_scanning_window_open = false"
        id="exit_preview_btn"
        tabindex="0"
    ...
    <div id="table-header">
      <button
        v-if="lectureIsOngoing()"
        @click="qr_scanning_window_open = true"
    ...
    <AnswerPoll
      v-if="answering_poll"
      :poll="current_poll"
      @answer="handleAnswerPoll"
      @cancel="handleCancelPoll"
    />
  </div>
</template>
  ...
  methods: {
    lectureIsOngoing() {
      ...
      return (
        ...
        Date.parse(this.lecture.start_time) <= now &&
        Date.parse(this.lecture.end_time) >= now
      );
    },
    canWatchRecording() {
      return (
        this.lecture.playback_submission_start_time &&
        Date.parse(this.lecture.playback_submission_start_time) < Date.now()
      );
    },
    lectureIsOver() {
      let now = Date.now();
    ...
    async createLiveSubmission() {
      ...
      this.submission.live_percent =
        this.submission.live_progress / this.lecture.checkins.length;
