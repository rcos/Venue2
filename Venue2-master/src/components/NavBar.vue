  ...
  <div>
    <nav id="venue-nav">
      ...
      <div id="venue-nav-links">
        ...
        >
          <img
            src="@/assets/venue-logo.svg"
            width="30"
            height="30"
            class="d-inline-block align-top"
            alt="Venue Logo"
            aria-label="Venue Logo"
          />
        </router-link>
        <!-- Dashboard Link -->
        <div class="venue-nav-link-container">
          <router-link
            class="venue-nav-link"
            :class="{ 'active-link': is_dashboard() }"
            :to="{ name: 'dashboard' }"
          >
            <p>Dashboard</p>
          </router-link>
          <div
            :class="'active-link-underline ' + (is_dashboard() ? 'active' : '')"
          ></div>
        </div>
        <!-- Courses Link -->
        <div
          class="venue-nav-link-container"
          id="instructor-course-dropdown"
          v-if="instructor_courses.length"
        >
          <a
            data-toggle="collapse"
            href="#instructor-collapse"
            class="venue-nav-link"
            :class="{ 'active-link': is_instructor_course_info() }"
            style="cursor:pointer;"
          >
            Instructor Courses
            <img
              src="@/assets/icons8-sort-down-26.png"
              width="10"
              height="10"
              alt="Down Icon"
          ...
          <hide-at breakpoint="mediumAndBelow">
            <div class="dropdown-content">
              <router-link
                ...
                :key="course._id"
                :to="{ name: 'course_info', params: { id: course._id } }"
              >
                <p>{{ course.name }}</p>
              </router-link>
            </div>
          ...
          <div
            :class="
              'active-link-underline ' +
                (is_instructor_course_info() ? 'active' : '')
            "
          ></div>
        </div>
        <div
          class="venue-nav-link-container"
          id="ta-section-dropdown"
          v-if="ta_sections.length"
        >
          <a
            data-toggle="collapse"
            href="#ta-collapse"
            class="venue-nav-link"
            :class="{ 'active-link': is_ta_section_info() }"
            style="cursor:pointer;"
          >
            TA Sections
            <img
          ...
          <hide-at breakpoint="mediumAndBelow">
            <div class="dropdown-content">
              <router-link
                v-for="section in ta_sections"
                :key="section._id"
                :to="{ name: 'course_info', params: { id: section._id } }"
              >
          ...
          <div
            :class="
              'active-link-underline ' + (is_ta_section_info() ? 'active' : '')
            "
          ></div>
        </div>
        <div
          class="venue-nav-link-container"
          id="student-section-dropdown"
          v-if="student_sections.length"
        >
          <a
            data-toggle="collapse"
            href="#student-collapse"
            class="venue-nav-link"
            :class="{ 'active-link': is_student_section_info() }"
            style="cursor:pointer;"
          >
            Student Sections
            <img
          ...
          <hide-at breakpoint="mediumAndBelow">
            <div class="dropdown-content">
              <router-link
                v-for="section in student_sections"
                :key="section._id"
                :to="{ name: 'course_info', params: { id: section._id } }"
              >
                <p>{{ section.course.name }} {{ section.name }}</p>
          ...
          <div
            :class="
              'active-link-underline ' +
                (is_student_section_info() ? 'active' : '')
            "
          ></div>
        </div>
        <!-- Statistics Link -->
        ...
        <div class="venue-nav-link-container">
            ...
            <show-at breakpoint="large">
              <p aria-label="User Name">
                ...
                <img
                  ...
                  width="20"
                  height="20"
                  class="d-inline-block align-top settings"
                  alt="Settings Icon"
          ...
          <div
            :class="'active-link-underline ' + (is_settings() ? 'active' : '')"
          ></div>
        </div>
      </div>
    </nav>
    ...
    <show-at breakpoint="mediumAndBelow">
      <div id="all-collapse">
        <div
          class="collapse"
          id="instructor-collapse"
          data-parent="#all-collapse"
        >
          <ul class="mobile-course-list">
            <li
              class="mobile-course-link"
              href="#instructor-collapse"
              data-toggle="collapse"
              v-for="course in instructor_courses"
              :key="course._id"
          ...
          <ul class="mobile-course-list">
            ...
            >
              <router-link
                :to="{ name: 'course_info', params: { id: section._id } }"
              >
                <p class="mobile-course-link-name">
    ...
    <div id="breadcrumb-container" v-if="showBreadcrumb()">
      ...
      <div v-if="current_section" class="crumb">
        ...
        />
        <router-link
          :to="{ name: 'course_info', params: { id: current_section._id } }"
        >
      ...
      <div v-else-if="current_course" class="crumb">
        ...
        />
        <router-link
          :to="{ name: 'course_info', params: { id: current_course._id } }"
        >
          {{ current_course.name }}
        </router-link>
      </div>
      <div v-if="current_lecture" class="crumb">
        ...
        <router-link
          :to="{
            name: 'lecture_info',
            params: { lecture_id: current_lecture._id }
          }"
        >
  ...
  methods: {
    showBreadcrumb() {
      return !(
        ...
        this.$route.name === "lecture_playback"
      );
    },
    async loadData() {
      ...
      const response4 = await LectureAPI.getLecturesForUser(
        this.current_user._id,
        "with_sections_and_course"
      );
    ...
    loadBreadcrumb() {
      this.current_lecture = this.user_lectures.find(
        a =>
          this.$route.params.lecture_id &&
          a &&
          a._id == this.$route.params.lecture_id
      );
      let sections = this.ta_sections.concat(this.student_sections);

      if (this.$route.name == "new_lecture") {
        this.current_course = this.instructor_courses.find(
          a =>
            this.$route.params.course_id &&
            a._id == this.$route.params.course_id
        );
      } else if (this.current_lecture) {
        this.current_section = this.current_lecture.sections.find(section =>
          sections.map(a => a._id).includes(section._id)
        );
        if (!this.current_section) {
          this.current_course = this.current_lecture.sections[0].course;
        }
      } else {
        this.current_section = sections.find(
          section => section._id == this.$route.params.id
        );
        if (!this.current_section) {
          this.current_course = this.instructor_courses.find(
            course => course._id == this.$route.params.id
          );
        }
      }
    },
    ...
    is_instructor_course_info() {
      return (
        this.$route.name === "course_info" &&
        this.instructor_courses.some(a => a._id == this.$route.params.id)
      );
    },
    is_ta_section_info() {
      return (
        this.$route.name === "course_info" &&
        this.ta_sections.some(a => a._id == this.$route.params.id)
      );
    },
    is_student_section_info() {
      return (
        this.$route.name === "course_info" &&
        this.student_sections.some(a => a._id == this.$route.params.id)
      );
    },
    is_lecture_info() {
...
#student-section-dropdown:focus-within > .dropdown-content a:nth-of-type(1) {
  ...
  width: 15rem;
  padding: 12px 16px;
  transition: opacity 0.25s 0s cubic-bezier(0.19, 1, 0.22, 1),
    max-height 0.25s 0s cubic-bezier(0.19, 1, 0.22, 1),
    padding 0.25s 0s cubic-bezier(0.19, 1, 0.22, 1),
    width 0.25s 0s cubic-bezier(0.19, 1, 0.22, 1);
}
...
#student-section-dropdown:focus-within > .dropdown-content a:nth-of-type(2) {
  ...
  width: 15rem;
  padding: 12px 16px;
  transition: opacity 0.25s 0.05s cubic-bezier(0.19, 1, 0.22, 1),
    max-height 0.25s 0.05s cubic-bezier(0.19, 1, 0.22, 1),
    padding 0.25s 0.05s cubic-bezier(0.19, 1, 0.22, 1),
    width 0.25s 0.05s cubic-bezier(0.19, 1, 0.22, 1);
}
...
#student-section-dropdown:focus-within > .dropdown-content a:nth-of-type(3) {
  ...
  width: 15rem;
  padding: 12px 16px;
  transition: opacity 0.25s 0.1s cubic-bezier(0.19, 1, 0.22, 1),
    max-height 0.25s 0.1s cubic-bezier(0.19, 1, 0.22, 1),
    padding 0.25s 0.1s cubic-bezier(0.19, 1, 0.22, 1),
    width 0.25s 0.1s cubic-bezier(0.19, 1, 0.22, 1);
}
...
#student-section-dropdown:focus-within > .dropdown-content a:nth-of-type(4) {
  ...
  width: 15rem;
  padding: 12px 16px;
  transition: opacity 0.25s 0.15s cubic-bezier(0.19, 1, 0.22, 1),
    max-height 0.25s 0.15s cubic-bezier(0.19, 1, 0.22, 1),
    padding 0.25s 0.15s cubic-bezier(0.19, 1, 0.22, 1),
    width 0.25s 0.15s cubic-bezier(0.19, 1, 0.22, 1);
}
...
#student-section-dropdown:focus-within > .dropdown-content a:nth-of-type(5) {
  ...
  transition: opacity 0.25s 0.2s cubic-bezier(0.19, 1, 0.22, 1),
    max-height 0.25s 0.2s cubic-bezier(0.19, 1, 0.22, 1),
    padding 0.25s 0.2s cubic-bezier(0.19, 1, 0.22, 1),
    width 0.25s 0.2s cubic-bezier(0.19, 1, 0.22, 1);
}
...
#student-section-dropdown:focus-within > .dropdown-content a:nth-of-type(6) {
  ...
  width: 15rem;
  padding: 12px 16px;
  transition: opacity 0.25s 0.25s cubic-bezier(0.19, 1, 0.22, 1),
    max-height 0.25s 0.25s cubic-bezier(0.19, 1, 0.22, 1),
    padding 0.25s 0.25s cubic-bezier(0.19, 1, 0.22, 1),
    width 0.25s 0.25s cubic-bezier(0.19, 1, 0.22, 1);
}
...
.dropdown-content a:focus {
  ...
  transition: all 0.25s cubic-bezier(0.19, 1, 0.22, 1);
  outline: none;
  box-shadow: 0px 3px 3px 0px rgba(85, 85, 85, 0.644) inset,
    0px -3px 3px 0px rgba(179, 179, 179, 0.644) inset;
}
