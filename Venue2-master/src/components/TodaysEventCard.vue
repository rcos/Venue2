<<template>
  <div>
    ...
    <hide-at breakpoint="small">
      <div
        class="todays-event-card-container"
        v-bind:class="{
          'active-container': is_active,
          'over-container': is_over
        }"
      >
        <div class="todays-event-card">
          <div
            class="spinner-border event-card-spinner"
            role="status"
            v-if="!section_has_loaded"
          >
            <span class="sr-only">Loading...</span>
          </div>
          <div v-else>
            <div class="event-title desktop-event-title">{{ event.title }}</div>
            <div class="course-title desktop-course-title">
              {{ event.section.course.dept }}
              {{ event.section.course.course_number }}-{{ event.section.name }}
            </div>
          </div>
        </div>
      </div>
    ...
    <show-at breakpoint="small">
      <div style="position: relative;">
        <div
          class="mobile-event-card-extending-background"
          v-bind:class="{
            'active-container': is_active,
            'over-container': is_over
          }"
        ...
        <div class="mobile-event-card">
          ...
          <div v-else>
            ...
            <div class="course-title mobile-course-title">
              {{ event.section.course.dept }}
              {{ event.section.course.course_number }}-{{ event.section.name }}
            </div>
          </div>
  ...
  methods: {
    ...
    setEventStatus() {
      ...
      if (current_time >= event_start_time) {
        if (current_time <= event_end_time) this.is_active = true;
        else this.is_over = true;
      }
    }
  }
};
