<template>
  <div id="lecture-upload-modal">
    ...
    >
      <img
        src="@/assets/icons8-upload-96.png"
        width="60"
        alt="QR Code"
        aria-label="QR Code"
      />
    ...
    <div id="lecture_modal_viewable" class="hiddenModal">
      <div class="row titlerow">
        <h1 id="banner_title" aria-label="New Lecture Video">
          New Lecture Video
          <button
            type="button"
            v-if="update_lecture"
      ...
      <div class="row youtuberow">
        ...
        <div v-if="video_type == 'video/youtube'">
          ...
          <input
            ...
            max="5"
            value="0"
            aria-labelledby="yt_length_label"
            placeholder="0 hours"
          />
          <input
            class="col-4"
      ...
      <div class="row filerow" v-if="video_type == ''">
        <input
          ...
          role="button"
          tabindex="0"
          aria-label="Select Video and Show Poll Creation Options"
        />
      </div>
      <div class="row" id="lecture_container" v-if="file_selected">
          ...
          <div class="poll_card">
            <div class="row questionrow">
              ...
              <textarea
                ...
                class="col"
                type="text"
                placeholder="eg. Which of the following...?"
                aria-labelledby="question_label"
              />
            </div>
            <div class="row">
              <div class="col-4">
            ...
            <ol class="row possible_answer">
              <li v-for="(current_answer, i) in current_answers" v-bind:key="i">
                ...
                <input
                  class="answerfield"
                  type="text"
                  v-model.lazy="current_answers[i]"
                  aria-labelledby="a_label"
                />
                <input
                  class="iscorrectfield"
                  type="checkbox"
                  v-model.lazy="current_is_correct[i]"
                  aria-labelledby="correct_label"
                />
                <button
                  type="button"
                  class="btn btn-danger removeanswer"
                  @click="
                    current_answers.splice(i, 1);
                    current_is_correct.splice(i, 1);
                  "
        ...
        <div
          class="col"
          v-if="
            (polls && polls.length) || (need_timestamp && need_timestamp.length)
          "
        >
          <h2>Current Polls</h2>
          ...
          <ol class="row pollrow">
            <li v-for="(poll, i) in polls" :key="i" class="row">
              ...
              <input
                ...
                :value="poll.question"
                readonly
                aria-labelledby="pq_label"
              />
              <button
                type="button"
                class="removepollbtn btn btn-danger"
                @click="polls.splice(i, 1)"
                :aria-label="'Remove Poll ' + (i + 1)"
              >
                X
              </button>
            </li>
            <li
              v-for="(poll, i) in need_timestamp"
              :key="i"
              class="row needs-timestamp-li"
            >
              <div class="col-3">
                <label id="hour_label">Hour</label>
                <input
              ...
              <div class="col-3">
                <button
                  type="button"
                  class="btn btn-primary get-time"
                  aria-label="Set Time From Video"
                  @click="getVideoTime(i)"
                >
                  Get Time
                </button>
              </div>
              <input
                class="needs-timestamp"
                :value="poll.question"
                readonly
                aria-labelledby="pq_label"
              />
      ...
      <div class="row" id="bottomrow">
        <div class="col">
          ...
          <div
            id="playback_start"
            aria-labelledby="playback_start_label"
            type="datetime-local"
          ></div>
        </div>
        <div class="col">
          ...
          <div
            id="playback_end"
            class=""
            aria-labelledby="playback_end_label"
            type="datetime-local"
          ></div>
        </div>
        ...
        <button
          ...
          v-if="update_lecture"
          id="video_upload_btn"
          class="btn btn-secondary uploadbtn"
          @click="updateLecture()"
        >
          Upload
        </button>
...
export default {
  ...
  methods: {
    ...
    async updateLecture() {
      if (this.isComplete()) {
        ...
        if (!this.lecture.video_type) {
          let response = await LectureAPI.addPlaybackVideo(
            this.lecture._id,
            lecture_video
          );
        ...
        if (this.need_timestamp.length == 0) {
          ...
          for (let i = 0; i < this.need_timestamp.length; i++) {
            ...
            PlaybackPollAPI.update(this.need_timestamp[i]).then(res => {
              n_saved++;
    ...
    async updateLectureFromParent(lect, course_id) {
      ...
      if (this.polls.length == 0) {
        ...
        this.$router.push({
          name: "course_info",
          params: {
            id: this.$store.state.user.current_user.ta_sections.includes(
        ...
        for (let i = 0; i < this.polls.length; i++) {
          ...
          PlaybackPollAPI.addPoll(this.polls[i]).then(res => {
            n_saved++;
            ...
            self.$nextTick(() => {
              if (self.vjs == null) {
                videojs("video_player", {}, function() {
                  self.vjs = this;
                  self.vjs.src({
                    type: vjs_type ? vjs_type : vid_selector.files[0].type,
                    src: url ? url : URL.createObjectURL(vid_selector.files[0])
                  });
                  self.vjs.load();
                });
              } else {
              ...
              self.play_sub_start_picker = new Picker(
                document.querySelector("#playback_start"),
                {
                  inline: true,
                  rows: 1,
                  headers: true,
              ...
              self.play_sub_end_picker = new Picker(
                document.querySelector("#playback_end"),
                {
                  inline: true,
                  rows: 1,
                  headers: true,
    ...
    hideModal() {
      ...
      document
        .getElementById("lecture_modal_viewable")
        .classList.add("hiddenModal");
    },
    showModal() {
    ...
    smhToTimestamp(poll) {
      return (
        parseInt(poll.sec) +
        parseInt(poll.min) * 60 +
        parseInt(poll.hour) * 60 * 60
      );
    },
    isComplete() {
      ...
      if (this.need_timestamp) {
        return this.need_timestamp.every(
          poll =>
            undefined != poll.sec &&
            undefined != poll.min &&
            undefined != poll.hour
    ...
    updateTimestamp(i) {
      this.need_timestamp[i].timestamp =
        parseInt(this.need_timestamp[i].hour) * 3600 +
        parseInt(this.need_timestamp[i].min) * 60 +
        parseInt(this.need_timestamp[i].sec);
    },
    getVideoTime(i) {
