  ...
  <div id="lecture-attendance-table">
    <div class="tabs">
      <button
        ...
        tabindex="0"
        @click="selectTab(0)"
        aria-label="Show Live Attendance"
        role="tab"
        aria-selected="true"
        aria-controls="live_submit"
      >
        <div
          id="live-progress-bar"
          :style="{ width: live_percent * 100 + '%' }"
          class="progress-bar"
        >
      ...
      >
        <LectureAttendanceList
          ...
          :lecture="lecture"
          :submissions_with_live="submissions_with_live"
          :is_live="true"
        />
      </div>
      ...
      <button
        ...
        tabindex="0"
        @click="selectTab(1)"
        aria-label="Show Playback Attendance"
        role="tab"
        aria-selected="false"
        aria-controls="playback_submit"
      >
        <div
          id="playback-progress-bar"
          :style="{ width: playback_percent * 100 + '%' }"
          class="progress-bar"
        >
          <h5>
      ...
      >
        ...
        >
          <h5>
            Asynchronous - {{ (playback_percent * 100).toFixedDecimals(1) }}%
          </h5>
        </div>
      </button>
      <div
      ...
      >
        <LectureAttendanceList
          :is_instructor="is_instructor"
          :lecture="lecture"
          :submissions_with_playback="submissions_with_playback"
          :is_playback="true"
        />
      </div>
      ...
      <button
        ...
        tabindex="0"
        @click="selectTab(2)"
        aria-label="Show Absent"
        role="tab"
        aria-selected="false"
        aria-controls="no_submit"
      >
        <div
          id="absent-progress-bar"
          :style="{ width: absent_percent * 100 + '%' }"
          class="progress-bar"
        >
          <h5>
            Absent -
            {{
              (
                (submissions_with_nothing.length / all_students.length) *
                100
              ).toFixedDecimals(1)
            }}%
          </h5>
      ...
      >
        <LectureAttendanceList
          :is_instructor="is_instructor"
          :lecture="lecture"
          :submissions_with_nothing="submissions_with_nothing"
          :is_absent="true"
        />
      </div>
      ...
      <button
        v-if="all_students"
        id="instructors_only_btn"
        class="tab_btn"
        tabindex="0"
        @click="selectTab(3)"
        aria-label="Show Instructors Only"
        role="tab"
        aria-selected="false"
        aria-controls="instructors_only"
      >
      ...
      <div
        v-if="selected_tab === 3"
        role="tabpanel"
        aria-labelledby="instructors_only_btn"
        id="instructors_only"
        class="tab_section"
      >
        <div id="manual-override-container">
          ...
          <div class="input-group">
            ...
            <input
              ...
              class="form-control"
              placeholder="eg: 'whitte3,mbizin'"
              aria-label="RCS IDs to override"
            />
            <div class="input-group-append">
              <button
                id="submit-manual-override"
        ...
        <div class="row justify-content-center">
          <div id="table-container" v-if="lecture.checkins.length > 0">
            ...
            <table
              id="checkins-container"
              aria-labelledby="checkin-table-label"
            >
              <thead>
                <tr>
                  <th>Start Time</th>
              ...
              <tbody>
                <tr v-for="(checkin, i) in lecture.checkins" :key="i">
                  <td v-if="checkin.start_time">
                    {{ getPrettyDateTime(new Date(checkin.start_time)) }}
                  </td>
                  <td v-else>Manual</td>
                  <td v-if="checkin.end_time">
                    {{ getPrettyDateTime(new Date(checkin.end_time)) }}
                  </td>
                  <td v-else>Manual</td>
                  <td v-if="polls[i] && polls[i].timestamp">
                    {{ polls[i].timestamp }}
                  </td>
                  <td v-else>X</td>
                  <td v-if="polls[i]">
                    <div
                      id="edit-poll-modal-container"
                      v-if="edit_poll_index != -1 && edit_poll_index == i"
                    >
                      <div id="edit-poll-modal-contents">
                        <CreatePoll
                          ...
                          :checkin="i"
                          :poll="polls[i]"
                          @cancel="handleCancelEditPoll"
                          @addPoll="handleEditPoll"
                        />
                      </div>
                    </div>
                    {{ polls[i].question }}
                    <button
                      ...
                      v-if="
                        !checkin.start_time ||
                          (checkin.start_time &&
                            Date.parse(checkin.start_time) > Date.now())
                      "
                      class="btn btn-secondary"
                      @click="edit_poll_index = i"
                ...
                <tr v-for="(poll, i) in polls" :key="i">
                  ...
                  <td>
                    ...
                    <button
                      type="button"
                      v-if="
                        Date.parse(lecture.playback_submission_start_time) >
                          Date.now()
                      "
                      class="btn btn-secondary"
                      @click="edit_poll_index = i"
  ...
  methods: {
    parseSubmissions() {
      ...
      if (this.all_students) {
        ...
        this.absent_percent =
          this.submissions_with_nothing.length / this.all_students.length;
      } else {
    ...
    handleOverride() {
      ...
      if (rcs_list.length == 1 && rcs_list[0] == "") {
        ...
        LectureSubmissionAPI.addLiveSubmissionByRCS(
          rcs_list,
          this.lecture._id
        ).then(res => {
    ...
    handleEditPoll(poll) {
      PlaybackPollAPI.update(poll).then(res => {
        location.reload();
      });
    }
  }
...
#absent_btn:hover,
#absent_btn:focus #absent_btn.selected_tab {
