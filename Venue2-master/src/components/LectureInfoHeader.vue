    ...
    <show-at breakpoint="mediumAndBelow">
      ...
      <div id="lecture-data-mobile">
        <div id="main-info-mobile">
          ...
          <div>
            {{ lecture.sections[0].course.name }}
            <p class="dept-and-number">
              {{ lecture.sections[0].course.prefix }}
              {{ lecture.sections[0].course.suffix }}
            </p>
          </div>
          <p class="sections">
            Sections: {{ lecture.sections.map(a => a.name).join(", ") }}
          </p>
        </div>
        <div class="row">
          ...
          <div v-if="lecture.playback_submission_start_time" class="col">
            ...
            <p>
              {{
                getPrettyDateTime(
                  new Date(lecture.playback_submission_start_time)
                )
              }}
            </p>
            <p>-</p>
            <p>
              {{
                getPrettyDateTime(
                  new Date(lecture.playback_submission_end_time)
                )
              }}
            </p>
          </div>
    ...
    <show-at breakpoint="large">
      <div id="lecture-data" class="row">
        ...
        <div v-if="lecture.start_time" class="col time-window my-auto">
          <h5>Live Lecture Window</h5>
          <p>
            {{ getPrettyDateTime(new Date(lecture.start_time)) }} -
            {{ getPrettyDateTime(new Date(lecture.end_time)) }}
          </p>
        </div>
        <div
          v-if="lecture.playback_submission_start_time"
          class="col time-window my-auto"
        >
          <h5>Video Attendance Window</h5>
          <p>
            {{
            ...
            -
            {{
              getPrettyDateTime(new Date(lecture.playback_submission_end_time))
            }}
          </p>
        </div>
      </div>
