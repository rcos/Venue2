      ...
      <tbody v-else>
        <tr v-for="event in events" :key="event._id">
          ...
          <div v-if="is_event_view">
            <td>
              <router-link
                :to="{ name: 'admin_edit_event', params: { id: event._id } }"
          ...
          <td v-else>
            <button
              ...
              @click.prevent="$emit('select-event', event)"
            >
              Select
            </button>
          </td>
...
export default {
  ...
  methods: {
    async loadEvents() {
      ...
      if (this.events.length === 0) this.sections_have_loaded = true;
      else this.getSectionsForEvents();
    },
    async deleteEvent(id) {
      const response = await EventAPI.deleteEvent(id);
      this.events.splice(
        this.events.findIndex(i => i._id == id),
        1
      );
    },
    async getSectionsForEvents() {
      ...
      this.events.forEach(async event => {
        ...
        counter++;
        if (counter == this.events.length) this.sections_have_loaded = true;
      });
    },
    setEventView() {
