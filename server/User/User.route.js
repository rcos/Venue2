...
userRoutes.route('/student_sections/:id').get(function (req, res) {
  ...
  User.findById(student_id, function (err, student) {
    if (err || student == null) {
      ...
      res.json(student.student_sections);
    }
  });
});
...
userRoutes.route('/instructors_for_course/:course_id').get(function (req, res) {
  ...
  Course.findById(course_id, function (err, course) {
    if (err || !course) {
      ...
      User.find({ email: { $in: course.instructors } }, function (err, instructors) {
        if (err || instructors == null) {
    ...
    if (err || sections == null) {
      ...
      sections.forEach(section => {
        students.push(new Promise((resolve, reject) => {
          User.find({ email: { $in: section.students } }, function (err, studs) {
            if (err || studs == null) {
              resolve(null);
            } else {
              resolve(studs);
          ...
          if (err || section == null) {
            ...
            student_emails.forEach(student_email => {
              User.find({ email: student_email }, function (err, student) {
                if (err || student == null) {
                  console.log("<ERROR> Getting user with Email User.route.js:", student_email)
                  res.json(err);
                } else {
                  let found = false;
